...
export default class MDEditor extends Vue {
  ...
  handleCodeScroll (percentage: number): void {
    const mdViewer: any = this.$refs.markdownViewer
    if (mdViewer) {
      const viewer: any = mdViewer.$refs.display
  ...
  insertContentWithoutSelection (text: string, selectionPart? : string): void {
    const editor: any = this.$refs.monacoEditor
    if (!editor) {
      return
    }
    const moEd: any = editor.monacoEditor
    if (!moEd) {
      return
    }
    let selection: any = moEd.getSelection()
    const startCol: number = selection.startColumn
    const startLine: number = selection.startLineNumber
    const endCol: number = selection.endColumn
    const endLine: number = selection.endLineNumber
    // 全部插入
    const range: any = new window.monaco.Range(startLine, startCol, endLine, endCol)
    const id: any = { major: 1, minor: 1 }
    const op: any = { identifier: id, range: range, text: text, forceMoveMarkers: true }
    // 调整选中部分
    const idx = text.indexOf(selectionPart || text[0])
    const len = selectionPart ? selectionPart.length : text.length
  ...
  insertContentWithSelection (textFront: string, textBack?: string): void {
    ...
    const op1: any = { identifier: id, range: range, text: textFront, forceMoveMarkers: false }
    // 头部插入
    range = new window.monaco.Range(startLine, startCol === 0 ? 0 : startCol, startLine, startCol)
    id = { major: 1, minor: 1 }
    const op2 = { identifier: id, range: range, text: textBack || textFront, forceMoveMarkers: false }
    // 调整选中部分
  ...
  getImageFromImageEditor (image: object): void {
